===========
Ping Parade
===========

:slug: pingparade
:template: page-project

.. image:: images/pingparade-banner.3.1.png
    :alt: Ping Parade
    :width: 960px
    :height: 150px

**There are more nodes in heaven and earth, my friends, than are dreamt of in our schematics.**

Let's go explore!
=================

.. raw:: html

    <ul class="rig columns-4">
        <li>
            <a class="invisiblelink" href="pingparade5.html"><img src="images/pingparade5-0-thumb.png" alt="#5 -- Raspberry" /></a>
            <p><a href="pingparade5.html">#5 -- Raspberry</a></p>
            <p>03 October 2014</p>
        </li>
        <li>
            <a class="invisiblelink" href="pingparade4.html"><img src="images/pingparade4-0-1-thumb.png" alt="#4 -- Breaker" /></a>
            <p><a href="pingparade4.html">#4 -- Breaker</a></p>
            <p>26 September 2014</p>
        </li>
        <li>
            <a class="invisiblelink" href="pingparade3.html"><img src="images/pingparade3-0-thumb.png" alt="#3 -- Backup" /></a>
            <p><a href="pingparade3.html">#3 -- Backup</a></p>
            <p>19 September 2014</p>
        </li>
        <li>
            <a class="invisiblelink" href="pingparade2.html"><img src="images/pingparade2-0-thumb.png" alt="#2 -- Keys" /></a>
            <p><a href="pingparade2.html">#2 -- Keys</a></p>
            <p>12 September 2014</p>
        </li>
        <li>
            <a class="invisiblelink" href="pingparade1.html"><img src="images/pingparade1-0-thumb.png" alt="#1 -- Repurpose" /></a>
            <p><a href="pingparade1.html">#1 -- Repurpose</a></p>
            <p>05 September 2014</p>
        </li>
    </ul>
